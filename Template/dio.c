...
/* TODO: Define implementation */
  ...
  for (uint16_t Channel = 0; Channel < DIO_MAX_PIN_NUMBER; Channel++)
    {
...
Dio_ChannelRead(DioChannel_t Channel)
{
  ...
  DioPinState_t PortState = (DioPinState_t)*Dio_PortsIn[Channel / DIO_CHANNELS_PER_PORT];
  /* Determine the port bit associated with this channel */
  DioPinState_t PinMask = (DioPinState_t)(1UL << (Channel % DIO_CHANNELS_PER_PORT));
...
* the channel and the DioPinState_t to define the desired state.<br>
...
* @param State is HIGH or LOW as defined in the DioPinState_t enum <br>
* \b Example:
* @code
...
**********************************************************************/
void 
Dio_ChannelWrite(DioChannel_t Channel, DioPinState_t State)
{
  if (State == HIGH)
    {
...
{
  //Assert that this address is in range of Dio addresses
  return *Address;
}
